...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, LineChart, Line, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar, AreaChart, Area } from 'recharts';
...
export default function FastTalkComparison() {
  ...
  const MetricCard = ({ title, qwen, llama, ministral, unit = 'ms', lower = true, highlight = false }) => {
    ...
    const getColor = (model) => {
      if (model === 'qwen') return winner === 'qwen' ? 'text-purple-400' : 'text-gray-400';
      if (model === 'llama') return winner === 'llama' ? 'text-cyan-400' : 'text-gray-400';
      return winner === 'ministral' ? 'text-amber-400' : 'text-gray-400';
    };
    
    return (
      <div className={`bg-gray-800 rounded-lg p-4 ${highlight ? 'ring-2 ring-yellow-500' : ''}`}>
        <h4 className="text-gray-400 text-sm mb-3">{title}</h4>
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <span className="text-purple-400 text-xs">Qwen3</span>
            <span className={`text-lg font-bold ${getColor('qwen')}`}>
              {qwen.toFixed(1)}{unit}
              {winner === 'qwen' && <span className="ml-2 text-xs bg-purple-500/20 px-1.5 py-0.5 rounded">✓</span>}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-cyan-400 text-xs">Llama3.1</span>
            <span className={`text-lg font-bold ${getColor('llama')}`}>
              {llama.toFixed(1)}{unit}
              {winner === 'llama' && <span className="ml-2 text-xs bg-cyan-500/20 px-1.5 py-0.5 rounded">✓</span>}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-amber-400 text-xs">Ministral3</span>
            <span className={`text-lg font-bold ${getColor('ministral')}`}>
              {ministral.toFixed(1)}{unit}
              {winner === 'ministral' && <span className="ml-2 text-xs bg-amber-500/20 px-1.5 py-0.5 rounded">✓</span>}
            </span>
          </div>
        </div>
      </div>
  ...
  const WinnerBadge = ({ model }) => {
    const colors = {
      qwen: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
      llama: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30',
      ministral: 'bg-amber-500/20 text-amber-400 border-amber-500/30'
    };
    const names = { qwen: 'Qwen3', llama: 'Llama3.1', ministral: 'Ministral3' };
    return (
      <span className={`text-xs px-2 py-0.5 rounded border ${colors[model]}`}>
        🏆 {names[model]}
      </span>
    );
  };

  const MetricRow = ({ label, qwen, llama, ministral, unit = 'ms', lower = true, highlight = false }) => {
    const values = { qwen, llama, ministral };
    ...
    const format = (v) => v === null || v === undefined ? '-' : typeof v === 'number' ? v.toFixed(1) : v;
    
    return (
      <tr className={`border-b border-gray-700/50 ${highlight ? 'bg-yellow-500/5' : ''}`}>
        <td className="py-3 px-3 font-medium">{label}</td>
        <td className={`text-right px-3 ${winner === 'qwen' ? 'text-purple-400 font-semibold' : ''}`}>
          {format(qwen)}{unit && qwen !== null ? unit : ''}
        </td>
        <td className={`text-right px-3 ${winner === 'llama' ? 'text-cyan-400 font-semibold' : ''}`}>
          {format(llama)}{unit && llama !== null ? unit : ''}
        </td>
        <td className={`text-right px-3 ${winner === 'ministral' ? 'text-amber-400 font-semibold' : ''}`}>
          {format(ministral)}{unit && ministral !== null ? unit : ''}
        </td>
        <td className="text-center px-3"><WinnerBadge model={winner} /></td>
      </tr>
    );
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-lg">Three-Model Comparison • December 5, 2025</p>
          <p className="text-gray-500 text-sm mt-1">Whisper large-v3-turbo • Kokoro TTS • Q4 Quantization • All Local</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {models.map((model) => (
            ...
            >
              ...
              <p className="text-gray-400 text-sm">{model.vendor} • Q4 Quantization</p>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-gray-500">Success:</span>
                  <span className={`ml-2 ${model.successRate === 100 ? 'text-green-400' : model.successRate >= 90 ? 'text-yellow-400' : 'text-orange-400'}`}>
                    {model.successRate}%
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">UPL:</span>
                  <span className="ml-2 text-white font-medium">{model.upl.median.toFixed(0)}ms</span>
                </div>
              </div>
            </div>
          ))}
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div className="bg-gray-800 rounded-lg p-6 mb-8">
              <h3 className="text-lg font-semibold mb-4">Overall Performance Profile</h3>
              ...
              <p className="text-gray-500 text-xs text-center mt-2">Higher = better (latencies inverted, consistency = 100 - σ)</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-purple-900/20 rounded-lg p-5 border border-purple-500/30">
                ...
                <ul className="text-sm text-gray-300 space-y-2">
                  ...
                  <li className="text-gray-500">⚠ Has one 929ms outlier</li>
                </ul>
              </div>
              <div className="bg-cyan-900/20 rounded-lg p-5 border border-cyan-500/30">
                ...
                <ul className="text-sm text-gray-300 space-y-2">
                  ...
                  <li className="text-gray-500">⚠ 4 failed long-question runs</li>
                </ul>
              </div>
              <div className="bg-amber-900/20 rounded-lg p-5 border border-amber-500/30">
                ...
                <ul className="text-sm text-gray-300 space-y-2">
                  ...
                  <li className="text-gray-500">⚠ Slowest overall (320ms median)</li>
                </ul>
              </div>
            </div>
          </>
        ...
        {activeTab === 'latency' && (
          <>
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              <h3 className="text-lg font-semibold mb-4">Median Latency Comparison (Lower = Better)</h3>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={comparisonBarData} layout="vertical" barGap={2}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                  <XAxis type="number" tick={{ fill: '#9ca3af' }} domain={[0, 550]} />
                  <YAxis dataKey="metric" type="category" tick={{ fill: '#9ca3af' }} width={100} />
                  <Tooltip 
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div className="bg-gray-800 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Short Question UPL per Run</h3>
                <ResponsiveContainer width="100%" height={280}>
                  <LineChart data={shortLineData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                    <XAxis dataKey="run" tick={{ fill: '#9ca3af' }} />
                    <YAxis tick={{ fill: '#9ca3af' }} domain={[240, 340]} />
                    <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '8px' }} formatter={(v) => [`${v.toFixed(1)}ms`]} />
                    <Legend />
              ...
              <div className="bg-gray-800 rounded-lg p-6">
                <h3 className="text-lg font-semibold mb-4">Medium Question UPL per Run</h3>
                <ResponsiveContainer width="100%" height={280}>
                  <LineChart data={mediumLineData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                    <XAxis dataKey="run" tick={{ fill: '#9ca3af' }} />
                    <YAxis tick={{ fill: '#9ca3af' }} domain={[250, 450]} />
                    <Tooltip contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '8px' }} formatter={(v) => [`${v.toFixed(1)}ms`]} />
                    <Legend />
                    <Line type="monotone" dataKey="qwen" name="Qwen3" stroke={COLORS.qwen} strokeWidth={2} dot={{ fill: COLORS.qwen, r: 4 }} />
                    <Line type="monotone" dataKey="llama" name="Llama3.1" stroke={COLORS.llama} strokeWidth={2} dot={{ fill: COLORS.llama, r: 4 }} />
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              <h3 className="text-lg font-semibold mb-4">Latency Summary by Question Type</h3>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-green-900/20 rounded-lg p-4 border border-green-500/30">
                  <h4 className="text-green-400 font-medium mb-2">Short Questions (~2.3s)</h4>
                  <p className="text-gray-400 text-xs mb-3">"What's the last thing that genuinely surprised you?"</p>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between"><span className="text-purple-400">Qwen3</span><span className="font-medium">278ms</span></div>
                    <div className="flex justify-between"><span className="text-amber-400">Ministral3</span><span>296ms</span></div>
                    <div className="flex justify-between"><span className="text-cyan-400">Llama3.1</span><span>306ms</span></div>
                  </div>
                </div>
                <div className="bg-yellow-900/20 rounded-lg p-4 border border-yellow-500/30">
                  <h4 className="text-yellow-400 font-medium mb-2">Medium Questions (~8.4s)</h4>
                  <p className="text-gray-400 text-xs mb-3">"If you could redesign one everyday object..."</p>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between"><span className="text-purple-400">Qwen3</span><span className="font-medium">291ms</span></div>
                    <div className="flex justify-between"><span className="text-cyan-400">Llama3.1</span><span>320ms</span></div>
                    <div className="flex justify-between"><span className="text-amber-400">Ministral3</span><span>378ms</span></div>
                  </div>
                </div>
                <div className="bg-red-900/20 rounded-lg p-4 border border-red-500/30">
                  <h4 className="text-red-400 font-medium mb-2">Long Questions (~16.8s)</h4>
                  <p className="text-gray-400 text-xs mb-3">"How would this reshape education, careers..."</p>
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between"><span className="text-purple-400">Qwen3</span><span>~650ms*</span></div>
                    <div className="flex justify-between"><span className="text-amber-400">Ministral3</span><span>~652ms*</span></div>
                    <div className="flex justify-between"><span className="text-cyan-400">Llama3.1</span><span className="text-gray-500">null**</span></div>
                  </div>
                  <p className="text-gray-500 text-xs mt-2">* Only measured when no streaming overlap<br/>** Audio started before speech ended</p>
                </div>
              </div>
            </div>
          </>
        ...
        {activeTab === 'variance' && (
          <>
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              <h3 className="text-lg font-semibold mb-4">Standard Deviation Comparison (Lower = More Consistent)</h3>
              ...
              <p className="text-gray-500 text-sm text-center mt-2">Llama 3.1 dominates consistency — 5x lower UPL variance than Qwen3 and Ministral3</p>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="text-gray-300">
                    <tr className="border-b border-gray-700/50">
                      ...
                      <td className="text-right text-green-400">69ms</td>
                    </tr>
                    <tr className="border-b border-gray-700/50">
                      <td className="py-2">TTSR</td>
                      <td className="text-right">254ms</td>
                      <td className="text-right">368ms</td>
                      <td className="text-right text-green-400">114ms</td>
                    </tr>
                    <tr>
                      <td className="py-2">STT FTL</td>
                      <td className="text-right">448ms</td>
                      <td className="text-right">610ms</td>
                      <td className="text-right text-yellow-400">162ms</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="text-gray-300">
                    <tr className="border-b border-gray-700/50">
                      ...
                      <td className="text-right text-yellow-400">167ms</td>
                    </tr>
                    <tr className="border-b border-gray-700/50">
                      <td className="py-2">TTSR</td>
                      <td className="text-right">211ms</td>
                      <td className="text-right">438ms</td>
                      <td className="text-right text-yellow-400">227ms</td>
                    </tr>
                    <tr>
                      <td className="py-2">STT FTL</td>
                      <td className="text-right">458ms</td>
                      <td className="text-right">546ms</td>
                      <td className="text-right text-green-400">88ms</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              <h3 className="text-lg font-semibold mb-4">Min/Max Extremes</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="text-gray-300">
                    <tr className="border-b border-gray-700/50">
                      <td className="py-3 px-3 text-purple-400 font-medium">Qwen3 8B</td>
                      <td className="text-right px-3 text-green-400">265ms</td>
                      <td className="text-right px-3 text-red-400">929ms</td>
                      <td className="text-right px-3 text-red-400">664ms</td>
                      <td className="text-center px-3 text-yellow-400">⚠ Wide</td>
                    </tr>
                    <tr className="border-b border-gray-700/50">
                      <td className="py-3 px-3 text-cyan-400 font-medium">Llama 3.1 8B</td>
                      <td className="text-right px-3">275ms</td>
                      <td className="text-right px-3 text-green-400">378ms</td>
                      <td className="text-right px-3 text-green-400">103ms</td>
                      <td className="text-center px-3 text-green-400">✓ Tight</td>
                    </tr>
                    <tr>
                      <td className="py-3 px-3 text-amber-400 font-medium">Ministral 3 8B</td>
                      <td className="text-right px-3 text-green-400">259ms</td>
                      <td className="text-right px-3 text-red-400">988ms</td>
                      <td className="text-right px-3 text-red-400">729ms</td>
                      <td className="text-center px-3 text-yellow-400">⚠ Wide</td>
                    </tr>
                  </tbody>
                </table>
        ...
        {activeTab === 'distribution' && (
          <>
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              <h3 className="text-lg font-semibold mb-4">UPL Distribution (All Measured Runs)</h3>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={uplDistribution}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                  <XAxis dataKey="range" tick={{ fill: '#9ca3af' }} label={{ value: 'UPL Range (ms)', position: 'insideBottom', offset: -5, fill: '#9ca3af' }} />
                  <YAxis tick={{ fill: '#9ca3af' }} label={{ value: 'Count', angle: -90, position: 'insideLeft', fill: '#9ca3af' }} />
            ...
            <div className="bg-gray-800 rounded-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="text-center">
                  ...
                  <h4 className="text-purple-400 font-medium mb-1">Qwen3 8B</h4>
                  <p className="text-gray-400 text-sm">Left-skewed distribution</p>
                  <p className="text-gray-500 text-xs mt-2">Most runs between 265-290ms with rare outliers up to 929ms</p>
                </div>
                <div className="text-center">
                  <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-cyan-500/20 flex items-center justify-center">
                    <span className="text-2xl">📊</span>
                  </div>
                  <h4 className="text-cyan-400 font-medium mb-1">Llama 3.1 8B</h4>
                  <p className="text-gray-400 text-sm">Normal distribution</p>
                  <p className="text-gray-500 text-xs mt-2">Tight clustering around 300-320ms, no extreme values</p>
                </div>
                <div className="text-center">
                  <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-amber-500/20 flex items-center justify-center">
                    <span className="text-2xl">📈</span>
                  </div>
                  <h4 className="text-amber-400 font-medium mb-1">Ministral 3 8B</h4>
                  <p className="text-gray-400 text-sm">Right-skewed distribution</p>
                  <p className="text-gray-500 text-xs mt-2">Wide spread from 259-988ms, higher values on medium questions</p>
                </div>
              </div>
            </div>

            <div className="bg-gray-800 rounded-lg p-6">
              <h3 className="text-lg font-semibold mb-4">Full Statistics Table</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-gray-400 border-b border-gray-700">
        ...
        {activeTab === 'raw-data' && (
          <div className="bg-gray-800 rounded-lg p-6">
            <h3 className="text-lg font-semibold mb-4">Complete Benchmark Data</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-gray-400 border-b border-gray-700">
                ...
                <tbody className="text-gray-300">
                  <tr className="bg-gray-700/30"><td colSpan="5" className="py-2 px-3 font-semibold text-gray-400">User Perceived Latency</td></tr>
                  ...
                  <tr className="bg-gray-700/30"><td colSpan="5" className="py-2 px-3 font-semibold text-gray-400">LLM Metrics</td></tr>
                  ...
                  <tr className="bg-gray-700/30"><td colSpan="5" className="py-2 px-3 font-semibold text-gray-400">STT Metrics</td></tr>
                  ...
                  <tr className="bg-gray-700/30"><td colSpan="5" className="py-2 px-3 font-semibold text-gray-400">Token Generation</td></tr>
                  ...
                  <tr className="bg-gray-700/30"><td colSpan="5" className="py-2 px-3 font-semibold text-gray-400">Reliability</td></tr>
        ...
        <div className="mt-8 bg-gradient-to-r from-gray-800 to-gray-700 rounded-lg p-6 border border-gray-600">
          ...
          <div className="grid grid-cols-3 gap-6 text-sm">
            <div>
              <h4 className="text-purple-400 font-semibold mb-2">Best for Speed</h4>
              <p className="text-gray-300">Qwen3 8B — 280ms median UPL, fastest across all question types. Trade-off: occasional outliers up to 929ms.</p>
            </div>
            <div>
              <h4 className="text-cyan-400 font-semibold mb-2">Best for Consistency</h4>
              <p className="text-gray-300">Llama 3.1 8B — σ=27ms variance, tightest P5-P95 range (69ms). Trade-off: slightly slower median (308ms).</p>
            </div>
            <div>
              <h4 className="text-amber-400 font-semibold mb-2">Best for Reliability</h4>
              <p className="text-gray-300">Ministral 3 8B — 100% success rate, no STT failures. Trade-off: slowest median (320ms) and verbose responses.</p>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-600 text-gray-400 text-sm">
            <strong>Overall:</strong> All three models achieve sub-440ms P95 UPL for conversational turns — well within the sub-1s target. 
            For production use, <strong className="text-cyan-400">Llama 3.1 8B</strong> offers the best UX due to predictable latency, 
            while <strong className="text-purple-400">Qwen3 8B</strong> is optimal when absolute speed matters more than consistency.
          </div>
        </div>
